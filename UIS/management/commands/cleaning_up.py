...
def clean_up_graduates(conflict, simulate):
    ...
    if not simulate:
        Student.objects.filter(registration_number__in=GRADUATES.keys()).update(
            status='G')
...
def clean_up_delete_students(conflict, simulate):
    ...
    for student_number, student in DELETED_STUDENTS.iteritems():
        ...
        StudentEnrolment.objects.filter(
            student_registration__student__registration_number=student_number
        ).delete()
        StudentEnrolmentLog.objects.filter(
            student_registration__student__registration_number=student_number
        ).delete()
        StudentRegistration.objects.filter(
            student__registration_number=student_number).delete()
        Student.objects.filter(registration_number=student_number).delete()
        i += 1


    if not simulate:
        Student.objects.filter(
            registration_number__in=NO_REGISTRATIONS.keys()).delete()
...
def clean_up_dropped_out_students(conflict, simulate):
    ...
    if not simulate:
        Student.objects.filter(registration_number__in=DROPPED_OUT.keys()).update(
            status='D')
